...
class Midia_model extends CI_Model{
    ...
    public function do_insert($dados=NULL, $redir=TRUE){
        if ($dados != NULL):
            ...
            if ($this->db->affected_rows()>0):
                auditoria('Inclusao de midia','Nova midia cadastrada no sistema');
     ...
     public function do_update($dados=NULL, $condicao=NULL, $redir=TRUE){
        if ($dados != NULL && is_array($condicao)):
            ...
            if ($this->db->affected_rows()>0):
                auditoria('Alteração de midia', 'A midia de id " '.$condicao['id'].' " foi alterada.');
    ...
    public function do_delete($condicao=NULL, $redir=TRUE){
        if ($condicao != NULL && is_array($condicao)):
            ...
            if ($this->db->affected_rows()>0):
                auditoria('Exclusão de midia', 'A midia de id " '.$condicao['id'].' " foi excluida.');
